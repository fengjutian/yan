...
export type Task = {ID:string;UserID:string;Type:string;Status:string;Prompt:string;ImageCount:number;CreatedAt:string}
export type Audit = {ID:string;AdminUserID:string;Action:string;ResourceType:string;ResourceID:string;DetailsJSON:string;CreatedAt:string}
...
async function request<T>(path:string, options:RequestInit={}):Promise<T>{
  ...
  if(response.status===401||response.status===403){localStorage.removeItem('admin_access_token');if(location.pathname!='/login')location.assign('/login')}
  if(!response.ok){const body=await response.json().catch(()=>null);throw new ApiError(response.status,body?.error?.message??'请求失败')}
...
export const api={
  ...
  saveStyle:(value:Partial<Style>,id?:string)=>request<Style>(id?`/api/v1/admin/styles/${id}`:'/api/v1/admin/styles',{method:id?'PUT':'POST',body:JSON.stringify({slug:value.Slug,name:value.Name,description:value.Description,prompt_template:value.PromptTemplate,negative_prompt:value.NegativePrompt??'',sort_order:value.SortOrder??0,enabled:value.Enabled??true})}),
  deleteStyle:(id:string)=>request<void>(`/api/v1/admin/styles/${id}`,{method:'DELETE'}),
  tasks:()=>request<Page<Task>>('/api/v1/admin/tasks'), audit:()=>request<Page<Audit>>('/api/v1/admin/audit-logs')
}
